% $Log: chapter13.tex,v $
% Revision 1.7  2004/09/04 02:37:58  teshields
...
\item \ixtt{-}, \ixtt{+}, (\ixtt{PLUS}, \ixtt{MINUS})
\item \ixtt{*}, \ixtt{/}, \verb|%|, \verb|%*|,\newline
\ixtt{OVER}, \ixtt{MOD}, \ixtt{ELEM}, (\ixtt{TIMES})
...
operator \verb|PLUS| in section \hyref{stan-othop} for addition with
a check for overflow.
\item \hx{\texttt{OP - = (L INT a,L INT b)L INT:}}{-@\texttt{-}!dyadic}\newline
Arithmetic subtraction: $a-b$. No check is made for integer
overflow. See the operator \verb|MINUS| in section \hyref{stan-othop} for
...
section \hyref{stan-othop} for multiplication with a check for overflow.
...
\item \hxtt{OP MINUS = (L INT a,b)L INT:}{MINUS}\newline
Subtracts \verb|b| from \verb|a|, checking for overflow.
\item \hxtt{OP PLUS = (L INT a,b)L INT:}{PLUS}\newline
Adds \verb|a| and \verb|b|, checking for overflow.
\item \hxtt{OP TIMES = (L INT a,b)L INT:}{TIMES}\newline
...
\item \hxtt{PROC(LONG INT,LONG INT,REF LONG INT)VOID ph lminus}{ph lminus}\newline
Subtracts two long integers.
\item \hxtt{PROC(LONG INT,LONG INT,REF LONG INT)VOID ph lplus}{ph lplus}\newline
Adds two long integers.
\item \hxtt{PROC(LONG INT,LONG INT,REF LONG INT)VOID ph ltimes}{ph ltimes}\newline
Multiplies two long integers.
\item \hxtt{PROC(INT,INT,REF INT)VOID ph minus}{ph minus}\newline
Subtracts two integers.
\item \hxtt{PROC(INT,INT,REF INT)VOID ph plus}{ph plus}\newline
Adds two integers.
\item \hxtt{PROC(INT,INT,REF INT)VOID ph times}{ph times}\newline
...
\ixtt{on signal} is declared as follows:-
\begin{verbatim}
...
operators \ixtt{PLUS}, \ixtt{MINUS} and \ixtt{TIMES}\hyref{stan-othop}
...
occur. For example, the declaration for \ixtt{PLUS} with
\ixtt{SHORT INT} operands is
\begin{verbatim}
...
\end{verbatim}
\noindent
Here is the mode of \ixtt{ansi raise}:
\begin{verbatim}
...
the routine returns \ixtt{estab invalid parameters} as error value. The
...
\hyref{stan-sockch}). The procedure yields zero on success, otherwise
